...
forward_kernel(float *y, const float *x, const float *k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    */
    ...
    int block_size = BLOCK_WIDTH + K - 1;
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    const int W_Grid = ceil(W_out / (float)BLOCK_WIDTH); //round up
    block_y = (blockIdx.z / W_Grid) * BLOCK_WIDTH;
    block_x = (blockIdx.z % W_Grid) * BLOCK_WIDTH;
    ...
    for (c = 0; c < C; c++)
    {
        ...
        for (int i = row_o; i < block_y + block_size; i += BLOCK_WIDTH)
        {
            for (int j = col_o; j < block_x + block_size; j += BLOCK_WIDTH)
            {
                if (i < H && j < W)
                {
...
void forward<gpu, float>(mshadow::Tensor<gpu, 4, float> &y, const mshadow::Tensor<gpu, 4, float> &x, const mshadow::Tensor<gpu, 4, float> &w)
{
    ...
    size_t shared_mem_ext_size = sizeof(float) * (BLOCK_WIDTH + K - 1) * (BLOCK_WIDTH + K - 1);
